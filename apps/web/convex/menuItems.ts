...
export const listByCategory = query({
  ...
  handler: async (ctx, args) => {
    const items = await ctx.db.query("menuItems").withIndex("by_category", (q) => q.eq("categoryId", args.categoryId)).collect();
...
export const listByRestaurant = query({
  ...
  handler: async (ctx, args) => {
    const items = await ctx.db.query("menuItems").withIndex("by_restaurant", (q) => q.eq("restaurantId", args.restaurantId)).collect();
...
export const remove = mutation({
  args: { restaurantId: v.id("restaurants"), id: v.id("menuItems") },
  handler: async (ctx, args) => {
    await requireRole(ctx, args.restaurantId, ["owner", "manager"]);
...
export const toggleAvailability = mutation({
  args: { restaurantId: v.id("restaurants"), id: v.id("menuItems") },
  handler: async (ctx, args) => {
    await requireRole(ctx, args.restaurantId, ["owner", "manager", "kitchen"]);
