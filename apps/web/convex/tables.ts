...
export const listByRestaurant = query({
  ...
  handler: async (ctx, args) => {
    const tables = await ctx.db.query("tables").withIndex("by_restaurant", (q) => q.eq("restaurantId", args.restaurantId)).collect();
...
export const getByQrCode = query({
  ...
  handler: async (ctx, args) => {
    const table = await ctx.db.query("tables").withIndex("by_qr_code", (q) => q.eq("qrCode", args.qrCode)).unique();
...
export const create = mutation({
  args: { restaurantId: v.id("restaurants"), number: v.number(), label: v.optional(v.string()), seats: v.number(), qrCode: v.string() },
  handler: async (ctx, args) => {
...
export const update = mutation({
  args: { restaurantId: v.id("restaurants"), id: v.id("tables"), number: v.optional(v.number()), label: v.optional(v.string()), seats: v.optional(v.number()), qrCode: v.optional(v.string()) },
  handler: async (ctx, args) => {
    await requireRole(ctx, args.restaurantId, ["owner", "manager"]);
...
export const updateStatus = mutation({
  args: { restaurantId: v.id("restaurants"), id: v.id("tables"), status: v.union(v.literal("available"), v.literal("occupied"), v.literal("reserved"), v.literal("cleaning")), currentOrderId: v.optional(v.id("orders")) },
  handler: async (ctx, args) => {
