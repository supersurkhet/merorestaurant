...
export const listByRestaurant = query({
  ...
  handler: async (ctx, args) => {
    const all = await ctx.db
      ...
      .collect();
    return all.filter((c) => c.isActive).sort((a, b) => a.sortOrder - b.sortOrder);
  },
});
...
export const remove = mutation({
  args: { restaurantId: v.id("restaurants"), id: v.id("categories") },
  handler: async (ctx, args) => {
    await requireRole(ctx, args.restaurantId, ["owner", "manager"]);
    const items = await ctx.db.query("menuItems").withIndex("by_category", (q) => q.eq("categoryId", args.id)).first();
